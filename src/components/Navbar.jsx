...
const Navbar = () => {
  ...
  return (
    <div className={`navbar top-0 left-1/2 transform -translate-x-1/2  ${scrollPos >= 5  && "blur_navbar"   } ${isListing && "listing_variant "}`}>
        <div className={`app__flex !justify-between max-w-[1240px] mx-auto gap-[80px] px-[12px] py-[28px] ${isListing && "!py-[16px]"}`}>
                ...
                <div className="flex gap-2  overflow-x-scroll">
                        {
                                ...
                                .map((item,i)=>(
                                        <button key={i} className={`text-[14px] whitespace-nowrap leading-[16px]  border-solid border-[#1E1E1E] border-[.2px] rounded-[4px] bg-[#FBFAFC] py-1 px-2 `}>{item}</button>
                                ))
                        }
                </div>
